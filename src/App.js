...
let port = process.env.PORT || "http://localhost:5000";
...
const App = () => {
  ...
  const fetchPolls = async () => {
    const res = await fetch(`${port}/polls`);
  ...
  const handleVote = (choice, pollId) => {
    ...
    axios
      .post(`${port}/polls/vote/${pollId}`, pollFromState)
  ...
  const handleCreate = (pollData) => {
    axios.post(`${port}/polls/add`, pollData).then((res) => {
